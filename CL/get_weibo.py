...
def request(name,index,max_id,token_container,lock):
    lock.acquire()
...
def get_weibo(name,token_container,lock,max_id=0):
    index = 1
    ...
    while True :
        try :
            weibo_data = request(name,index,max_id,token_container,lock)
        except urllib2.HTTPError :
            lock.acquire()
            new_token = token_container.return_new_token()
            lock.release()
